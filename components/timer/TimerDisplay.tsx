...
export const TimerDisplay: React.FC<TimerDisplayProps> = ({ mode, timeStr, isActive, totalSeconds, timeLeft, canvasRef, isPiP, isCompact, onToggleCompact, scale = 1 }) => {
    ...
    if (isCompact) {
        return (
            <div className="w-full h-full relative flex items-center justify-center">
                {/* Top Row: Mini Controls - FIXED POSITION */}
                {isPiP && (
                    <button
                        onClick={onToggleCompact}
                        className="fixed top-2 right-2 p-1 text-white/50 hover:text-white z-50 mix-blend-difference"
                        title="Expand"
                    >
    ...
    return (
        <div className="w-full h-full relative flex items-center justify-center">
            {/* PiP Minimize Button - Fixed to Window Top-Right OUTSIDE scaled container */}
            {isPiP && (
                <button
                    onClick={onToggleCompact}
